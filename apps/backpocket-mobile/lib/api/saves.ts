...
import { hasProcessingSaves, PROCESSING_POLL_INTERVAL_MS } from "./use-processing-saves";
...
export function useListSaves(input: ListSavesInput = {}) {
  ...
  return useInfiniteQuery({
    ...
    queryFn: async ({ pageParam }) => {
      return client.query<ListSavesInput, ListSavesResponse>("space.listSaves", {
        ...input,
        cursor: pageParam,
      });
    },
    ...
    refetchInterval: (query) => {
      ...
      return hasProcessingSaves(firstPageSaves) ? PROCESSING_POLL_INTERVAL_MS : false;
    },
  });
}
...
export function useGetSave(saveId: string) {
  ...
  return useQuery({
    ...
    refetchInterval: (query) => {
      ...
      const isProcessing =
        save.title === null && Date.now() - new Date(save.createdAt).getTime() < 60000;
...
export function useGetSaveSnapshot(saveId: string, includeContent = true) {
  ...
  return useQuery({
    ...
    queryFn: () =>
      client.query<GetSaveSnapshotInput, GetSaveSnapshotResponse>("space.getSaveSnapshot", {
        saveId,
        includeContent,
      }),
    enabled: !!saveId,
...
export function useToggleFavorite() {
  ...
  return useMutation({
    mutationFn: ({ saveId, value }: { saveId: string; value?: boolean }) =>
      client.mutate<{ saveId: string; value?: boolean }, Save>("space.toggleFavorite", {
        saveId,
        value,
      }),
    onMutate: async ({ saveId, value }) => {
      ...
      const previousDetail = queryClient.getQueryData<Save>(savesKeys.detail(saveId));
        ...
        return {
          ...
          pages: old.pages.map((page) => ({
            ...
            items: page.items.map((save) =>
              save.id === saveId ? { ...save, isFavorite: value ?? !save.isFavorite } : save
            ),
          })),
        };
      });
    ...
    onError: (_err, { saveId }, context) => {
      ...
      if (context?.previousDetail) {
        queryClient.setQueryData(savesKeys.detail(saveId), context.previousDetail);
      }
...
export function useToggleArchive() {
  ...
  return useMutation({
    mutationFn: ({ saveId, value }: { saveId: string; value?: boolean }) =>
      client.mutate<{ saveId: string; value?: boolean }, Save>("space.toggleArchive", {
        saveId,
        value,
      }),
    onMutate: async ({ saveId, value }) => {
      ...
      const previousDashboard = queryClient.getQueryData<DashboardData>(spaceKeys.dashboard());
      ...
      queryClient.setQueriesData<{
        ...
        return {
          ...
          pages: old.pages.map((page) => ({
            ...page,
            items: page.items.map((save) =>
              save.id === saveId ? { ...save, isArchived: value ?? !save.isArchived } : save
            ),
          })),
        };
      });
      ...
      if (previousDashboard) {
        ...
        if (isArchiving) {
          ...
          queryClient.setQueryData<DashboardData>(spaceKeys.dashboard(), {
            ...previousDashboard,
            recentSaves: previousDashboard.recentSaves.filter((save) => save.id !== saveId),
            stats: {
...
export function useDeleteSave() {
  ...
  return useMutation({
    mutationFn: (saveId: string) =>
      client.mutate<{ saveId: string }, { success: boolean }>("space.deleteSave", { saveId }),
    onMutate: async (saveId) => {
      ...
      if (previousDashboard) {
        const deletedSave = previousDashboard.recentSaves.find((s) => s.id === saveId);
        queryClient.setQueryData<DashboardData>(spaceKeys.dashboard(), {
          ...previousDashboard,
          recentSaves: previousDashboard.recentSaves.filter((save) => save.id !== saveId),
          stats: {
...
export function useCheckDuplicate() {
  ...
  return useMutation({
    mutationFn: (input: CheckDuplicateInput) =>
      client.mutate<CheckDuplicateInput, CheckDuplicateResponse>("space.checkDuplicate", input),
  });
}
...
export function getDuplicateSaveFromError(error: unknown): DuplicateSaveInfo | null {
  ...
  if (error.status === 409 || data?.code === "CONFLICT" || data?.httpStatus === 409) {

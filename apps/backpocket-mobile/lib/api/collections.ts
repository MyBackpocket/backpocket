...
export function useGetCollection(collectionId: string) {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      const collections = await client.query<void, Collection[]>("space.listCollections");
...
export function useCreateCollection() {
  ...
  return useMutation({
    mutationFn: (input: CreateCollectionInput) =>
      client.mutate<CreateCollectionInput, Collection>("space.createCollection", input),
    onMutate: async (newCollection) => {
      // Cancel outgoing refetches
      await queryClient.cancelQueries({ queryKey: collectionsKeys.lists() });

      // Snapshot previous value
      const previousCollections = queryClient.getQueryData<Collection[]>(collectionsKeys.list());
    ...
    onError: (_err, _newCollection, context) => {
      ...
      if (context?.previousCollections) {
        queryClient.setQueryData(collectionsKeys.list(), context.previousCollections);
      }
    },
    onSettled: () => {
...
export function useUpdateCollection() {
  ...
  return useMutation({
    mutationFn: (input: UpdateCollectionInput) =>
      client.mutate<UpdateCollectionInput, Collection>("space.updateCollection", input),
    onSuccess: (data) => {
...
export function useDeleteCollection() {
  ...
  return useMutation({
    mutationFn: (collectionId: string) =>
      client.mutate<{ collectionId: string }, { success: boolean }>("space.deleteCollection", {
        collectionId,
      }),

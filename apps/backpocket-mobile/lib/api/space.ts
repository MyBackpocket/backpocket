...
export const spaceKeys = {
  ...
  domainStatus: (domainId: string) => [...spaceKeys.all, "domainStatus", domainId] as const,
};
...
export function useDashboard() {
  ...
  return useQuery({
    ...
    refetchInterval: (query) => {
      ...
      return hasProcessingSaves(data.recentSaves) ? PROCESSING_POLL_INTERVAL_MS : false;
    },
  });
}
...
export function useCheckSlugAvailability() {
  ...
  return useMutation({
    mutationFn: (slug: string) =>
      client.mutate<{ slug: string }, SlugAvailability>("space.checkSlugAvailability", { slug }),
  });
}
...
export function useDomainStatus(domainId: string) {
  ...
  return useQuery({
    ...
    queryFn: () =>
      client.query<{ domainId: string }, DomainStatusResponse>("space.getDomainStatus", {
        domainId,
      }),
    enabled: !!domainId,
  });
...
export function useRemoveDomain() {
  ...
  return useMutation({
    mutationFn: (domainId: string) =>
      client.mutate<{ domainId: string }, { success: boolean }>("space.removeDomain", { domainId }),
    onSuccess: () => {

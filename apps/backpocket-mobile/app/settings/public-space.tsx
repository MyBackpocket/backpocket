...
export default function PublicSpaceSettingsScreen() {
  ...
  const isLoadingDomains = domains === undefined;
  ...
  // Use Convex query for slug availability (reactively updates)
  const slugToCheck =
    isEditingSlug &&
    debouncedSlug !== space?.slug &&
    debouncedSlug.length >= MIN_SLUG_LENGTH &&
    debouncedSlug.length <= MAX_SLUG_LENGTH &&
    SLUG_REGEX.test(debouncedSlug)
      ? debouncedSlug
      : undefined;
  const slugCheckResult = useCheckSlugAvailability(slugToCheck);

  // Update local state when slug check result changes
  useEffect(() => {
    if (!isEditingSlug || debouncedSlug === space?.slug) {
    ...
    if (debouncedSlug.length < MIN_SLUG_LENGTH) {
      setSlugAvailability({ available: false, reason: "too_short" });
      setIsCheckingSlug(false);
      return;
    }
    ...
    // Convex query handles the async check
    if (slugCheckResult === undefined) {
      setIsCheckingSlug(true);
    } else {
      setSlugAvailability(slugCheckResult);
      setIsCheckingSlug(false);
    }
  }, [debouncedSlug, isEditingSlug, space?.slug, slugCheckResult]);
        ...
        [
          { text: "Cancel", style: "cancel" },
          {
            ...
            onPress: async () => {
              ...
              try {
                await removeDomainAction({ domainId: domain.id as DomainId });
              } catch (error) {
  ...
  return (
    ...
    >
      ...
      >
        {/* Public Space Toggle */}
        <Text style={[styles.sectionTitle, { color: colors.mutedForeground }]}>Visibility</Text>
            ...
            <View style={styles.row}>
              <View
                style={[
                  ...
                  {
                    backgroundColor: isPublicEnabled ? `${brandColors.mint}30` : colors.muted,
                  },
                ]}
              >
                {isPublicEnabled ? (
              ...
              <View style={styles.rowContent}>
                <Text style={[styles.rowLabel, { color: colors.text }]}>Public Space</Text>
        ...
        <Card style={styles.card}>
          <CardContent style={styles.cardContent}>
            <View style={styles.row}>
              <View
                style={[styles.iconContainer, { backgroundColor: `${brandColors.rust.DEFAULT}15` }]}
              >
                <Bookmark size={20} color={brandColors.rust.DEFAULT} strokeWidth={2} />
              </View>
              <View style={styles.rowContent}>
                <Text style={[styles.rowLabel, { color: colors.text }]}>New saves default to</Text>
                <Text style={[styles.rowDescription, { color: colors.mutedForeground }]}>
                ...
                return (
                  ...
                  >
                    {isPublicOption ? (
                      <Unlock
                        size={16}
                        color={isSelected ? brandColors.mint : colors.mutedForeground}
                        strokeWidth={2}
                      />
                    ) : (
                      <Lock
        ...
        <Text style={[styles.helpText, { color: colors.mutedForeground }]}>
          This applies to quick saves from the share extension and new saves. You can always change
          visibility for individual saves.
        </Text>
        ...
        {isPublicEnabled && (
          <>
              ...
              >
                <View style={styles.previewHeader}>
                  ...
                  <View style={styles.previewHeaderText}>
                    ...
                    <Text style={[styles.previewUrl, { color: brandColors.teal }]}>
                      {publicUrl}
                    </Text>
                  </View>
                </View>
                ...
                <View style={styles.previewStats}>
                  <View style={styles.previewStat}>
                    ...
                    <Text style={[styles.previewStatText, { color: colors.mutedForeground }]}>
                ...
                <View style={styles.previewActions}>
                  ...
                  >
                    ...
                    <Text style={styles.previewButtonTextPrimary}>Preview Space</Text>
                  </TouchableOpacity>
                </View>
              </LinearGradient>
            </View>
            ...
            <Text style={[styles.helpText, { color: colors.mutedForeground }]}>
              Share your public link with others to let them view your public saves and collections.
              Only saves marked as public will be visible.
            </Text>
                ...
                {isEditingSlug ? (
                  <>
                    <View style={styles.slugEditRow}>
                      ...
                      />
                      <Text style={[styles.slugSuffix, { color: colors.mutedForeground }]}>
                        .{ROOT_DOMAIN}
                      </Text>
                    </View>
                    ...
                    {(isCheckingSlug ||
                      slugAvailability ||
                      (slugInput.length > 0 && slugInput.length < MIN_SLUG_LENGTH)) && (
                      <View style={styles.availabilityRow}>
                        {isCheckingSlug ? (
                          <>
                            <ActivityIndicator size="small" color={colors.mutedForeground} />
                            <Text
                              style={[styles.availabilityText, { color: colors.mutedForeground }]}
                            >
                              Checking...
                            </Text>
                          </>
                          ...
                          <>
                            {slugAvailability.available ? (
                              <CheckCircle2 size={14} color={brandColors.mint} strokeWidth={2.5} />
                            ) : (
                              <XCircle size={14} color={colors.destructive} strokeWidth={2.5} />
                            )}
                            <Text
                              style={[
                    ...
                    <View style={styles.editActions}>
                      ...
                      <TouchableOpacity
                        style={[
                          styles.editButton,
                          {
                            backgroundColor: canSaveSlug ? colors.primary : colors.muted,
                        ...
                        {isUpdatingSlug ? (
                          ...
                          <Text
                            style={[
                              ...
                              {
                                color: canSaveSlug ? "#FFFFFF" : colors.mutedForeground,
                              },
                            ]}
                          >
                            Save
                  ...
                  <View style={styles.slugDisplayRow}>
                    <View style={styles.slugDisplay}>
                      <Text style={[styles.slugText, { color: colors.text }]}>{space?.slug}</Text>
                      <Text style={[styles.slugSuffix, { color: colors.mutedForeground }]}>
                        .{ROOT_DOMAIN}
                      </Text>
                    </View>
                    <TouchableOpacity
                      ...
                      activeOpacity={0.7}
                    >
                      <Text style={[styles.changeButtonText, { color: colors.text }]}>Change</Text>
                    </TouchableOpacity>
                  </View>
                )}
              </CardContent>
                      ...
                      <View style={styles.domainRow}>
                        <View style={styles.domainInfo}>
                          ...
                          <View style={styles.domainStatus}>
                            ...
                            <Text style={[styles.domainStatusText, { color: statusInfo.color }]}>
                              {statusInfo.label}
                            </Text>
                          </View>
                        </View>
              ...
              <CardContent style={styles.cardContent}>
                <View style={styles.webOnlyContainer}>
                  ...
                  <View style={styles.webOnlyContent}>
                    ...
                    <Text style={[styles.webOnlyDescription, { color: colors.mutedForeground }]}>
        ...
        {!isPublicEnabled && (
          <Text style={[styles.helpText, { color: colors.mutedForeground }]}>
            When disabled, no one can view your public space. Enable it to share your saves with
            others.
          </Text>
        )}
      </ScrollView>

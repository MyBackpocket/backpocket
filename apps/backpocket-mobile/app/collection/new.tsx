...
export default function NewCollectionScreen() {
  ...
  const toggleSaveSelection = useCallback((saveId: string) => {
    setSelectedSaveIds((prev) =>
      prev.includes(saveId) ? prev.filter((id) => id !== saveId) : [...prev, saveId]
    );
  }, []);
    ...
    <>
      ...
      >
        ...
        >
          ...
          <Card style={styles.nameCard}>
            <CardContent style={styles.nameCardContent}>
              <View style={styles.nameInputGroup}>
                <View style={[styles.iconPreview, { backgroundColor: colors.muted }]}>
              ...
              {defaultTags.length > 0 && (
                <View style={styles.defaultTagsRow}>
                  {defaultTags.map((tagName) => (
                    <View
                      key={tagName}
                      style={[styles.defaultTagChip, { backgroundColor: `${brandColors.amber}20` }]}
                    >
                      <Text style={[styles.defaultTagText, { color: brandColors.amber }]}>
                        {tagName}
                      </Text>
          ...
          <Card style={styles.section}>
            <CardContent style={styles.sectionContent}>
              <Text style={[styles.sectionLabel, { color: colors.text }]}>Add Saves</Text>
                ...
                <View style={styles.selectedSaves}>
                  {selectedSaveIds.map((saveId) => {
                    ...
                    return (
                      ...
                      >
                        <Text
                          style={[styles.selectedSaveText, { color: brandColors.denim.deep }]}
                          numberOfLines={1}
                        >
                          {save.title || save.url}
                        </Text>
                        <TouchableOpacity onPress={() => toggleSaveSelection(saveId)} hitSlop={8}>
                    ...
                    return (
                      <Pressable
                        ...
                        style={[
                          styles.saveItem,
                          {
                            backgroundColor: isSelected ? `${brandColors.amber}15` : colors.muted,
                            borderColor: isSelected ? brandColors.amber : "transparent",
                          },
                        ]}
                      ...
                      >
                        <View style={styles.saveItemContent}>
                          <Text
                            style={[styles.saveItemTitle, { color: colors.text }]}
                            numberOfLines={1}
                          >
                            {save.title || save.url}
                          </Text>
                          <Text
                            style={[styles.saveItemUrl, { color: colors.mutedForeground }]}
                            numberOfLines={1}
                          >
                            {save.siteName || new URL(save.url).hostname}
                          </Text>
                        </View>
                        {isSelected && <Check size={18} color={brandColors.amber} />}
                      </Pressable>
                    );
                  })}
                </View>
              ...
              <Text style={[styles.hint, { color: colors.mutedForeground }]}>
                You can add saves to this collection later from the save detail screen
              </Text>
            </CardContent>
          </Card>

...
export default function SavesScreen() {
  ...
  const { filter, tagId, tagName, collectionId, collectionName } = useLocalSearchParams<{
  ...
  const hasActiveFilters =
    activeQuickFilter !== "all" || activeTagId || activeCollectionId || appliedSearchQuery;
    ...
    return (
      <View style={styles.emptyState}>
        <View style={[styles.iconContainer, { backgroundColor: colors.muted }]}>
          {appliedSearchQuery ? (
            <Search size={48} color={colors.mutedForeground} strokeWidth={1.5} />
          ) : activeTagId ? (
            <TagIcon size={48} color={colors.mutedForeground} strokeWidth={1.5} />
          ) : activeCollectionId ? (
            <FolderOpen size={48} color={colors.mutedForeground} strokeWidth={1.5} />
        ...
        {!hasActiveFilters && (
          <Button onPress={() => router.push("/save/new")} style={styles.addButton}>
            <Plus size={20} color="#FFFFFF" />
            <Text style={styles.addButtonText}>Add Save</Text>
          </Button>
        )}
        {hasActiveFilters && (
          <Button variant="secondary" onPress={clearAllFilters} style={styles.addButton}>
            <Text style={[styles.addButtonText, { color: colors.text }]}>Clear Filters</Text>
          </Button>
        )}
      </View>
    );
  ...
  const renderHeader = useCallback(() => {
    if (!isFetching || !hasLoadedOnce.current || isManualRefreshing) return null;
    return (
  ...
  if (isError) {
    return (
      <View style={[styles.container, styles.centered, { backgroundColor: colors.background }]}>
        <Text style={[styles.errorText, { color: colors.destructive }]}>Failed to load saves</Text>
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      >
        <Text style={[styles.filterLabel, { color: colors.mutedForeground }]}>Showing:</Text>
        ...
        >
          ...
          {activeQuickFilter !== "all" ? (
            ...
            <TouchableOpacity
              ...
              activeOpacity={0.7}
            >
              <Text style={[styles.filterChipText, { color: colors.text }]}>All Saves</Text>
            </TouchableOpacity>
          ) : null}
          ...
          {activeTagId && activeTagName && (
            <TouchableOpacity
              onPress={openFilterModal}
              style={[styles.filterChip, { backgroundColor: `${brandColors.denim.DEFAULT}20` }]}
              activeOpacity={0.7}
            >
              <TagIcon size={12} color={brandColors.denim.deep} />
              <Text style={[styles.filterChipText, { color: brandColors.denim.deep }]}>
                {activeTagName}
              </Text>
              <TouchableOpacity
          ...
          {activeCollectionId && activeCollectionName && (
            <TouchableOpacity
              onPress={openFilterModal}
              style={[styles.filterChip, { backgroundColor: `${brandColors.teal}20` }]}
              activeOpacity={0.7}
            >
              <FolderOpen size={12} color={brandColors.teal} />
              <Text style={[styles.filterChipText, { color: brandColors.teal }]}>
                {activeCollectionName}
              </Text>
              <TouchableOpacity
          ...
          {appliedSearchQuery && (
            <TouchableOpacity
              ...
              style={[styles.filterChip, { backgroundColor: `${brandColors.rust.DEFAULT}20` }]}
              activeOpacity={0.7}
            >
              <Search size={12} color={brandColors.rust.DEFAULT} />
              <Text
                style={[styles.filterChipText, { color: brandColors.rust.DEFAULT }]}
                numberOfLines={1}
              >
                "{appliedSearchQuery}"
              </Text>
              <TouchableOpacity
                ...
                hitSlop={{ top: 8, bottom: 8, left: 8, right: 8 }}
              >
                <X size={14} color={brandColors.rust.DEFAULT} strokeWidth={2.5} />
              </TouchableOpacity>
            </TouchableOpacity>
          )}
        </ScrollView>
        ...
        <TouchableOpacity
          ...
          style={[
            styles.filterButton,
            {
              backgroundColor: isSearchExpanded ? `${brandColors.rust.DEFAULT}20` : colors.muted,
            },
          ]}
          activeOpacity={0.7}
        >
          <Search
            size={18}
            color={isSearchExpanded ? brandColors.rust.DEFAULT : colors.mutedForeground}
          />
        </TouchableOpacity>
        ...
        <TouchableOpacity
          ...
          style={[
            styles.filterButton,
            {
              backgroundColor: hasActiveFilters ? `${colors.primary}20` : colors.muted,
            },
          ]}
          activeOpacity={0.7}
        >
          <Filter size={18} color={hasActiveFilters ? colors.primary : colors.mutedForeground} />
        </TouchableOpacity>
      </View>
          ...
          >
            ...
            <ScrollView style={styles.bottomSheetScroll} showsVerticalScrollIndicator={false}>
              ...
              <View style={styles.filterSection}>
                ...
                {tags && tags.filter((t) => (t._count?.saves ?? 0) > 0).length > 0 ? (
              ...
              <View style={styles.filterSection}>
                ...
                {collections && collections.filter((c) => (c._count?.saves ?? 0) > 0).length > 0 ? (
  ...
  return (
    <Card style={styles.saveCard}>
      <TouchableOpacity onPress={onPress} activeOpacity={0.7}>
        ...
        {save.imageUrl && (
          <Image source={{ uri: save.imageUrl }} style={styles.saveImage} resizeMode="cover" />
        )}
        ...
        <View style={styles.saveContent}>
          ...
          <View style={styles.siteInfo}>
            ...
            {save.visibility === "public" && (
              <View style={[styles.visibilityBadge, { backgroundColor: `${brandColors.mint}20` }]}>
                <Text style={[styles.visibilityText, { color: brandColors.mint }]}>Public</Text>
              </View>
            )}
          </View>

          {/* Title */}
          <Text style={[styles.saveTitle, { color: colors.text }]} numberOfLines={2}>
            {save.title || save.url}
          </Text>
          ...
          {save.description && (
            <Text
              style={[styles.saveDescription, { color: colors.mutedForeground }]}
              numberOfLines={2}
            >
              {save.description}
            </Text>
          ...
          {save.tags && save.tags.length > 0 && (
            <View style={styles.tags}>
              {save.tags.slice(0, 3).map((tag) => (
                <View
                  key={tag.id}
                  style={[styles.tag, { backgroundColor: `${brandColors.denim.DEFAULT}15` }]}
                >
                  <Text style={[styles.tagText, { color: brandColors.denim.deep }]}>
                    {tag.name}
                  </Text>
                </View>
              ))}
      ...
      <View style={[styles.actions, { borderTopColor: colors.border }]}>
        ...
        <TouchableOpacity onPress={onToggleArchive} style={styles.actionButton} activeOpacity={0.7}>
          <Archive size={20} color={save.isArchived ? brandColors.teal : colors.mutedForeground} />
        </TouchableOpacity>

        <TouchableOpacity onPress={handleOpenUrl} style={styles.actionButton} activeOpacity={0.7}>

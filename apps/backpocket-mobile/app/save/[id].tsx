...
function VisibilitySelector({ value, onChange, colors, showHint = true }: VisibilitySelectorProps) {
  const options: {
    value: SaveVisibility;
    label: string;
    icon: typeof Eye;
      ...
      <View style={visibilityStyles.container}>
        {options.map((option) => {
          ...
          return (
            ...
            >
              <Icon size={16} color={isSelected ? "#141D22" : colors.mutedForeground} />
              <Text
                style={[
              ...
              {isSelected && <Check size={14} color="#141D22" style={{ marginLeft: 2 }} />}
            </Pressable>
          );
        })}
      </View>
      {showHint && selectedOption && (
        <Text style={[visibilityStyles.hint, { color: colors.mutedForeground }]}>
          {selectedOption.hint}
        </Text>
      )}
    </View>
...
function TagChip({ name, onRemove }: TagChipProps) {
  return (
    <View style={[tagChipStyles.chip, { backgroundColor: `${brandColors.denim.DEFAULT}20` }]}>
      <Text style={[tagChipStyles.text, { color: brandColors.denim.faded }]}>{name}</Text>
      {onRemove && (
...
function CollectionChip({ name, isSelected, onToggle, colors }: CollectionChipProps) {
  return (
    <Pressable
      onPress={onToggle}
      style={[
    ...
    >
      <Text style={[collectionChipStyles.text, { color: isSelected ? "#141D22" : colors.text }]}>
        {name}
      </Text>
...
function EditSaveModal({ visible, onClose, save, colors }: EditSaveModalProps) {
  ...
  const tagSuggestions = useMemo(() => {
    ...
    return existingTags.filter((t) => !lowerTagNames.includes(t.name.toLowerCase())).slice(0, 5);
  ...
  return (
    ...
    >
      ...
      >
        <View style={[modalStyles.keyboardInner, { backgroundColor: colors.background }]}>
          {/* Header */}
          <View style={[modalStyles.header, { borderBottomColor: colors.border }]}>
            ...
            <Text style={[modalStyles.headerTitle, { color: colors.text }]}>Edit save</Text>
            <View style={{ width: 24 }} />
          </View>
  ...
  if (isError || !save) {
    return (
      <>
        ...
        />
        <View style={[styles.container, styles.centered, { backgroundColor: colors.background }]}>
          <Text style={[styles.errorText, { color: colors.destructive }]}>Save not found</Text>
      ...
      <>
        <Stack.Screen
          options={{
            ...
            headerRight: () => (
              <TouchableOpacity onPress={handleOpenUrl} style={styles.headerButton}>
        ...
        <View style={[styles.container, styles.centered, { backgroundColor: colors.background }]}>
          <View style={styles.processingContainer}>
            ...
            />
            <Text style={[styles.processingTitle, { color: colors.text }]}>Processing Save</Text>
            <Text style={[styles.processingDescription, { color: colors.mutedForeground }]}>
    ...
    <>
      ...
      >
        ...
        <View style={styles.mainInfo}>
          {/* Title */}
          <Text style={[styles.title, { color: colors.text }]}>{save.title || "Untitled"}</Text>

          {/* URL */}
          <TouchableOpacity onPress={handleOpenUrl}>
            <Text style={[styles.url, { color: brandColors.rust.DEFAULT }]} numberOfLines={2}>
              {save.url}
            </Text>
          </TouchableOpacity>
            ...
            <CardContent style={styles.readerModeCardContent}>
              ...
              >
                <View style={styles.readerModeHeaderLeft}>
                  ...
                  <Text style={[styles.readerModeTitle, { color: colors.text }]}>Reader Mode</Text>
                </View>
                <View style={styles.readerModeHeaderRight}>
                  {snapshotData.snapshot.wordCount && (
                    <View style={styles.readingTimeBadge}>
                      <Clock size={12} color={colors.mutedForeground} />
                      <Text style={[styles.readingTimeText, { color: colors.mutedForeground }]}>
                ...
                <View style={styles.readerModeContent}>
                  ...
                  {snapshotData.snapshot.byline && (
                    <View style={styles.readerByline}>
                      ...
                      />
                      <Text style={[styles.readerBylineText, { color: colors.mutedForeground }]}>
                        {snapshotData.snapshot.byline}
                      </Text>
                    </View>
                  )}
                  ...
                  {/* Divider */}
                  <View style={[styles.readerDivider, { backgroundColor: colors.border }]} />
        ...
        {isLoadingSnapshot && !snapshotData && (
          <Card style={styles.readerModeCard}>
            <CardContent style={styles.readerModeLoadingContent}>
              <ActivityIndicator size="small" color={brandColors.denim.DEFAULT} />
              <Text style={[styles.readerModeLoadingText, { color: colors.mutedForeground }]}>
                Loading reader mode...
              </Text>
            </CardContent>
          </Card>
          ...
          <CardContent style={styles.metadataContent}>
            ...
            <View style={styles.metadataRow}>
              ...
              <Text style={[styles.metadataValue, { color: colors.text }]}>
                {save.visibility.charAt(0).toUpperCase() + save.visibility.slice(1)}
              </Text>
            </View>
            ...
            {save.siteName && (
              <View style={[styles.metadataRow, { borderTopColor: colors.border }]}>
                ...
                <Text style={[styles.metadataValue, { color: colors.text }]}>{save.siteName}</Text>
              </View>
            )}
            ...
            <View style={[styles.metadataRow, { borderTopColor: colors.border }]}>
              <BookOpen size={18} color={colors.mutedForeground} />
              <Text style={[styles.metadataLabel, { color: colors.mutedForeground }]}>Saved</Text>
              <Text style={[styles.metadataValue, { color: colors.text }]}>
            ...
            {save.tags && save.tags.length > 0 ? (
              <View style={styles.tags}>
                {save.tags.map((tag) => (
                  <View
                    key={tag.id}
                    style={[styles.tag, { backgroundColor: `${brandColors.denim.DEFAULT}20` }]}
                  >
                    <Text style={[styles.tagText, { color: brandColors.denim.faded }]}>
                      {tag.name}
                    </Text>
                  </View>
                ))}
            ...
            {save.collections && save.collections.length > 0 ? (
              <View style={styles.collections}>
                {save.collections.map((collection) => (
                  <View
                    key={collection.id}
                    style={[styles.collection, { backgroundColor: colors.muted }]}
                  >
                    <FolderOpen size={16} color={colors.mutedForeground} />
                    <Text style={[styles.collectionText, { color: colors.text }]}>
                      {collection.name}
                    </Text>
                  </View>
                ))}
        ...
        <View style={styles.actionsZone}>
          ...
          >
            <Archive
              size={18}
              color={save.isArchived ? brandColors.denim.DEFAULT : colors.mutedForeground}
            />
